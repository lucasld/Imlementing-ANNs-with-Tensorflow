...
def train_step(model, input, target, loss_function, optimizer):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = loss_function(target, prediction)# + np.random.rand()
        gradients = tape.gradient(loss, model.trainable_variables)
        # apply gradients to the trainable variables using a optimizer
        optimizer.apply_gradients(zip(gradients, model.trainable_variables))
        accuracy = accuracy_function(prediction, target)
...
def accuracy_function(pred, target, acc_type='binary'):
    ...
    :param acc_type: Type of accuracy measure, defaults to 'binary'
    :type acc_type: String, optional
    :return: accuracy between pred and target
    :rtyype: float
    """
    if acc_type == 'binary':
        same_prediction = np.round(target,2) == np.round(pred,2)
    elif acc_type == 'argmax':
        same_prediction = np.argmax(target, axis=1) == np.argmax(pred, axis=1)
    else:
        print(f"{acc_type} is the wrong accuracy funcion type")
        return None
    return np.mean(same_prediction)



def training(input_model, datasets, loss_function, optimizer, epochs=10, variational=False):
    ...
    for epoch in range(1, epochs+1):
        ...
        for input, target in datasets['train']:
            if not variational:
                ...
                losses = input_model.train_step(input, target, loss_function, optimizer)
                reconstruction_loss, kl_loss, train_loss = losses
                train_accuracy = 0
